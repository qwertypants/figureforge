...
const useImageStore = create((set, get) => ({
  ...
  // Actions
  updateGenerationForm: (updates) => set((state) => ({
    generationForm: { ...state.generationForm, ...updates }
  })),
  
  resetGenerationForm: () => set(() => ({
    generationForm: {
      batch_size: 1,
      body_type: 'average',
      pose_type: 'standing',
  ...
  // Gallery actions
  setPublicImages: (images, append = false) => set((state) => ({
    publicImages: append ? [...state.publicImages, ...images] : images
  })),
  
  setUserImages: (images, append = false) => set((state) => ({
    userImages: append ? [...state.userImages, ...images] : images
  })),
  
  setFavorites: (favorites) => set({ favorites }),
  
  addToFavorites: (imageId) => set((state) => ({
    favorites: [...state.favorites, imageId]
  })),
  
  removeFromFavorites: (imageId) => set((state) => ({
