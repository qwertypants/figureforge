...
export const imagesAPI = {
  ...
  getPublicGallery: async (
    ...
    const response = await api.get<ImagesResponse>("/images/public", {
      params,
    });
    return response.data;
  },
  ...
  getFavorites: async (
    ...
    const params = cursor ? { cursor } : {};
    const response = await api.get<ImagesResponse>("/images/favorites", {
      params,
    });
    return response.data;
  },
  ...
  reportImage: async (
    ...
    details: string,
  ): Promise<ReportImageResponse> => {
    const response = await api.post<ReportImageResponse>(
